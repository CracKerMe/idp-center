...
export async function authenticateToken(
  ...
  try {
    ...
    jwt.verify(token, config.JWT_SECRET, async (err: jwt.VerifyErrors | null, decoded: jwt.JwtPayload | string | undefined) => {
      if (err || !decoded || typeof decoded === 'string') return res.status(401).json(error('Invalid token', ErrorCode.TOKEN_INVALID));

      if (typeof decoded !== 'object' || typeof decoded.id !== 'string' || typeof decoded.username !== 'string') {
