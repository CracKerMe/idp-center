 ...
 */
router.get('/health/comprehensive', authenticateAdmin, async (_req, res) => {
  try {
 ...
 */
router.post('/health/persist', authenticateAdmin, async (_req, res) => {
  try {
    const result = await runHealthCheck();
 ...
 */
router.post('/auto-heal/tick', authenticateAdmin, async (_req, res) => {
  try {
    await autoHealTick();
