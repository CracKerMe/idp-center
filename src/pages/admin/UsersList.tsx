        ...
        <div className="border-t border-zinc-200 px-4 py-5 sm:p-6 bg-zinc-50">
          <form onSubmit={handleCreate} className="space-y-4 max-w-lg">
            ...
            <div className="flex items-center">
              ...
              <label htmlFor="is_admin_create" className="ml-2 block text-sm text-zinc-900">Admin Privileges</label>
            </div>
      ...
      <div className="border-t border-zinc-200">
        <table className="min-w-full divide-y divide-zinc-200">
          <thead className="bg-zinc-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-zinc-500 uppercase tracking-wider">Username</th>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-zinc-500 uppercase tracking-wider">2FA</th>
          ...
          <tbody className="bg-white divide-y divide-zinc-200">
            {users.map((user) => (
              <tr key={user.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-500">{user.is_admin ? 'Admin' : 'User'}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-500">{user.otp_enabled ? 'Enabled' : 'Disabled'}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-500">
          ...
          <div className="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full sm:p-6 relative z-10">
              <form onSubmit={handleEdit}>
                <h3 className="text-lg font-medium text-zinc-900 mb-4">Edit User: {editingUser.username}</h3>
                {actionError && (
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center">
                    <input id="edit_is_admin" type="checkbox" checked={editForm.is_admin}
                      ...
                      className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-zinc-300 rounded" />
                    <label htmlFor="edit_is_admin" className="ml-2 block text-sm text-zinc-900">Admin Privileges</label>
                  </div>
                </div>
