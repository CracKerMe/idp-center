...
export default function AuditLogs() {
  ...
  return (
    <div className="bg-white shadow overflow-hidden sm:rounded-lg">
      ...
      {/* Filters */}
      <div className="border-t border-zinc-200 px-4 py-4 sm:px-6 bg-zinc-50">
        <form onSubmit={handleSearch} className="flex flex-wrap gap-3 items-end">
          <div>
            ...
            <select
              ...
              className="border border-zinc-300 rounded-md shadow-sm py-1.5 px-2 text-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
            >
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-600 mb-1">User ID</label>
            <input
              type="text"
              placeholder="User ID..."
              value={filters.user_id}
              onChange={e => setFilters(f => ({ ...f, user_id: e.target.value }))}
              className="border border-zinc-300 rounded-md shadow-sm py-1.5 px-2 text-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 w-44"
            />
          </div>
          <div>
          ...
          <button type="button" onClick={handleReset}
            className="inline-flex items-center px-3 py-1.5 border border-zinc-300 text-sm font-medium rounded-md text-zinc-700 bg-white hover:bg-zinc-50">
            Reset
          </button>
        </form>
      </div>
        ...
        <table className="min-w-full divide-y divide-zinc-200">
          <thead className="bg-zinc-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-zinc-500 uppercase tracking-wider">IP Address</th>
          ...
          <tbody className="bg-white divide-y divide-zinc-200">
            {loading ? (
              ...
              <tr><td colSpan={5} className="py-10 text-center text-sm text-zinc-500">No logs found</td></tr>
            ) : logs.map((log) => (
              <tr key={log.id}>
      ...
      {/* Pagination */}
      <div className="border-t border-zinc-200 px-4 py-3 flex items-center justify-between sm:px-6">
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={page === 1}
            className="inline-flex items-center px-2 py-1 border border-zinc-300 rounded-md text-sm text-zinc-700 bg-white hover:bg-zinc-50 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            <ChevronLeft className="h-4 w-4" />
          </button>
          <span className="inline-flex items-center px-3 py-1 text-sm text-zinc-700">
            {page} / {totalPages}
          </span>
          <button
