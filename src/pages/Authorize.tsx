...
export default function Authorize({ user }: { user: any }) {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-md bg-white dark:bg-zinc-900 py-8 px-4 shadow sm:rounded-lg sm:px-10 text-center">
          <XCircle className="mx-auto h-12 w-12 text-red-500" />
          <h3 className="mt-2 text-lg font-medium text-zinc-900 dark:text-white">Authorization Error</h3>
  ...
  if (!clientInfo) return <div className="min-h-screen flex items-center justify-center dark:bg-zinc-950 dark:text-white">Loading...</div>;
        ...
        <div className="bg-white dark:bg-zinc-900 py-8 px-4 shadow sm:rounded-lg sm:px-10">
          <div className="text-center">
            <p className="text-sm text-zinc-600 dark:text-zinc-400">
              <strong className="text-zinc-900 dark:text-white">{clientInfo.client_name}</strong> wants to access your account.
            </p>
            
            <div className="mt-4 bg-zinc-50 dark:bg-zinc-800 rounded-md p-4 text-left">
              <h4 className="text-sm font-medium text-zinc-900 dark:text-white">This application will be able to:</h4>
            ...
            <p className="mt-4 text-xs text-zinc-500 dark:text-zinc-400">
              Signed in as{' '}
              <strong className="text-zinc-900 dark:text-white">{user?.username ?? 'Guest'}</strong>
            ...
            <div className="mt-6 flex gap-4">
              <button
                ...
                className="flex-1 py-2 px-4 border border-zinc-300 dark:border-zinc-700 rounded-md shadow-sm text-sm font-medium text-zinc-700 dark:text-zinc-300 bg-white dark:bg-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              >
                Cancel
              </button>
              <button
